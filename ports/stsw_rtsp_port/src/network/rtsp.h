...
typedef struct RTSP_session {
    ...
    /**
     * @brief Pool of one thread for filling up data for the rtsp session
     *
     ...
     *
     * Please note that this is created during @ref rtsp_do_play
     * rather than during @ref rtsp_session_new, and deleted during
     * @ref rtsp_do_pause (and eventually during @ref
     * rtsp_session_free), so that we don't have reading threads to go
     * around during seeks.
     */
...
typedef struct {
     ...
     * @brief request body.
     *
     * This points to the buffer contains the request's body, 
     * if the request has no body(body is empty), it's NULL 
     */
    char *body;
