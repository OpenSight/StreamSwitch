...
static RTSP_Request *rtsp_parse_request(RTSP_Client *rtsp)
{
    ...
    /*
    printf("method %s, id: %d, object: %s, version: %s\n",
           req->method, (int)req->method_id, req->object, req->version);
    */
    ...
    if(message_length > (request_line_length+headers_length)) {
        req->body = g_strdup(message_data + request_line_length + headers_length);
    ...
    while (ml <= rtsp->input->len) {
        ...
        if (!mb) {    /* content length token not yet encountered. */
            if (!g_ascii_strncasecmp ((char*)rtsp->input->data + ml, eris_hdr_content_length,
                 ...
                 strlen(eris_hdr_content_length) < (RTSP_BUFFERSIZE - ml)?
                 strlen(eris_hdr_content_length) : (RTSP_BUFFERSIZE - ml) )) {
...
static void rtsp_handle_request(RTSP_Client *rtsp)
{
    ...
    static const rtsp_method_function methods[RTSP_ID_METHOD_NUM] = {
        ...
        [RTSP_ID_GET_PARAMETER]   = RTSP_get_parameter              
    };
    ...
    if(req->method_id < 0 || 
       req->method_id >= RTSP_ID_METHOD_NUM ||
       methods[req->method_id] == NULL) {
        rtsp_quick_response(req, RTSP_NotImplemented);
        return;
    }
